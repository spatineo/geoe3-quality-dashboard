...
from src.loader import load_API, load_dataset_metadata, load_csv
from src.evaluate import evaluate, evaluate_categories

current_dir = os.getcwd()

def read_rules_from_json(json_file, spatineo_api_access_key):
    try:
        with open(json_file, "r") as f:
            structure_file = json.load(f)

        # Update the 'url_start' value with the provided 'spatineo_api_access_key'
        structure_file["extractionRule"]["url_start"] = structure_file[
            "extractionRule"
        ]["url_start"].format(spatineo_api_access_key_json=spatineo_api_access_key)
                                                        ...
                                                        for (
                                                            ...
                                                            if (
                                                                ...
                                                                row = {
                                                                    ...
                                                                    "Metric_Weight": value9[
                                                                        "weight"
                                                                    ],
                                                                    "Metric_Description": value9[
                                                                        "description"
                                                                    ],
                                                                    "Extraction_Rule": value9[
                                                                        "extractionRule"
                                                                    ],
                                                                    "Evaluation_Rule": value9[
                                                                        "evaluationRule"
                                                                    ],
                                                                }
                                                                if func:
                                                                    if (
                                                                        ...
                                                                        model = {
                                                                            "dataset-metadata": load_dataset_metadata(
                                                                                metadata_file
                                                                            ),
                                                                            "service-availability": load_dataset_metadata(
                                                                                api_file
                                                                            ),
                                                                            "quality-evaluation": qualityEvaluation_file,
                                                                            "interoperability-maturity-model": interoperability_file,
...
def execute_xpath_rule(rule, model, file, serviceId=None):
    ...
    if value_method == "text":
        ...
        logging.warning(
            "Unknown value extraction method (%s) in extractionRule", value_method
        )
    return result
    ...
    for row in rows:
        if len(row) > 1 and row[1] == formula:
            if len(row) > 2:
                ...
                return result
    return (
        0  # Because if nothing is returned by the process, it means no error was found
    )

def execute_dot_notation_rule(rule, model, metadata, serviceId):
    # Your implementation of dot-notation extraction goes here
    pass

extractor_by_type = {
    'xpath': execute_xpath_rule,
    'formula': execute_formula_rule,
    'dot-notation': execute_dot_notation_rule,
}
...
if __name__ == "__main__":
    ...
    qualityEvaluation_file = os.path.join(current_dir, "src/buildings_and_errors/results_NO_cc.csv")
    serviceId = input(
        "Please enter the service ID (list of service Id : 39859,164572,157386,88383,157353) :"
    )
    interoperability_file = os.path.join(current_dir, "src/interoperability_maturityModel.csv")


   # Read the JSON file structuring the dashboard
    structure_file = read_rules_from_json(
        os.path.join(current_dir, "Dashboard_structure.json")
    )
    ...
    extractionRule_table = extract_all_info(
        structure_file,
        metadata_file,
        serviceId,
        qualityEvaluation_file,
